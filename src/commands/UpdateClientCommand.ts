...
export class UpdateClientCommand {
  ...
  public async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      const trafficResponse = await this.serverManager.getClientTrafficById(serverId, uuid);
      
      if (!trafficResponse.success || !trafficResponse.obj || trafficResponse.obj.length === 0) {
        const errorEmbed = new EmbedBuilder()
          .setColor(0xFF0000)
      ...
      const email = interaction.options.getString('email') || existingClient.email;
      ...
      if (expiryDays !== null) {
        expiryTime = expiryDays > 0 ? Date.now() + (expiryDays * 24 * 60 * 60 * 1000) : 0;
      }
      ...
      const response = await this.serverManager.updateClient(serverId, uuid, inboundId, updatedClient);
      ...
      if (response.success) {
        const embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: 'Total GB', value: totalGB === null ? 'Unchanged' : (totalGB === 0 ? 'Unlimited' : `${totalGB} GB`), inline: true },
            { name: 'Expiry', value: expiryDays === null ? 'Unchanged' : (expiryDays === 0 ? 'No expiry' : `${expiryDays} days`), inline: true },
            { name: 'Status', value: enabled === null ? 'Unchanged' : (enabled ? 'Enabled' : 'Disabled'), inline: true }
          )
          .setTimestamp();
